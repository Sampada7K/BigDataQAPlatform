...
class FileInputProcess:
    ...
    def process(self, region, station, start_date, end_date, filename,
        ...
        print start_date
        d1 = datetime.strptime(start_date, "%m/%d/%Y")
        print end_date
        d2 = datetime.strptime(end_date, "%m/%d/%Y")
        delta = abs(d2-d1)
        _days = delta.days


        print ("We are calculating default params for : ", _days)
        result['DefaultQualityParameters'] = qpDefaultObj.calculate_parameters(parameters,_days)
