...
class QPCalculation:
    ...
    def get_validity(self):
        ...
        for key in self.doc:
            ...
            if key == "SpCond":
                invalid = invalid + self.coll.find({'$or': [{key: {'$lt': 0}}, {key: {'$gt': 21.5}}]}).count()
            if key == "Sal":
                invalid = invalid + self.coll.find({'$or': [{key: {'$lt': 0}}, {key: {'$gt': 12.5}}]}).count()
            if key == "DO_pct":
            ...
            if key == "pH":
                invalid = invalid + self.coll.find({'$or': [{key: {'$lt': 0}}, {key: {'$gt': 7.5}}]}).count()
            if key == "Turb":
    ...
    def calculate_parameters(self, params, days):
        ...
        self.parameters["Overall_Data_Quality"] = float("{0:.2f}".format(overall_data_quality))
